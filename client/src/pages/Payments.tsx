/**
 * Payments Page
 * صفحة نظام الدفع والفواتير
 * 
 * @module ./client/src/pages/Payments
 */
import { Button } from "@/components/ui/button";
import { CreditCard, Plus, Search, Download, Eye, CheckCircle, Clock, AlertCircle, TrendingUp } from "lucide-react";
import { useState } from "react";

export default function Payments() {
  const [searchTerm, setSearchTerm] = useState("");

  const payments = [
    {
      id: "INV-001",
      date: "2026-02-07",
      customer: "شركة النقل السريع",
      amount: 1250,
      status: "مدفوع",
      statusColor: "bg-green-500/20 text-green-300",
      method: "تحويل بنكي"
    },
    {
      id: "INV-002",
      date: "2026-02-06",
      customer: "مستوردات الأردن",
      amount: 890,
      status: "قيد الانتظار",
      statusColor: "bg-yellow-500/20 text-yellow-300",
      method: "بطاقة ائتمان"
    },
    {
      id: "INV-003",
      date: "2026-02-05",
      customer: "الشركة الدولية",
      amount: 2150,
      status: "متأخر",
      statusColor: "bg-red-500/20 text-red-300",
      method: "شيك"
    }
  ];

  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-blue-900 to-slate-900 text-white">
      {/* Header */}
      <div className="sticky top-0 z-40 bg-slate-900/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-200 to-cyan-200">
                نظام الدفع والفواتير
              </h1>
              <p className="text-blue-200/60 mt-2">دفع آمن مع إدارة شاملة للفواتير</p>
            </div>
            <Button className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:shadow-lg hover:shadow-blue-500/50">
              <Plus className="w-4 h-4 mr-2" />
              فاتورة جديدة
            </Button>
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        {/* Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-green-600/20 to-green-700/20 border border-green-500/30 rounded-2xl p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-green-200/60 text-sm">المدفوعات المكتملة</p>
                <p className="text-3xl font-bold text-green-300 mt-2">$45,230</p>
                <p className="text-green-400 text-xs mt-2">↑ 12% عن الشهر السابق</p>
              </div>
              <CheckCircle className="w-10 h-10 text-green-400/50" />
            </div>
          </div>

          <div className="bg-gradient-to-br from-yellow-600/20 to-yellow-700/20 border border-yellow-500/30 rounded-2xl p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-yellow-200/60 text-sm">قيد الانتظار</p>
                <p className="text-3xl font-bold text-yellow-300 mt-2">$8,950</p>
                <p className="text-yellow-400 text-xs mt-2">3 فواتير</p>
              </div>
              <Clock className="w-10 h-10 text-yellow-400/50" />
            </div>
          </div>

          <div className="bg-gradient-to-br from-red-600/20 to-red-700/20 border border-red-500/30 rounded-2xl p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-200/60 text-sm">المتأخرة</p>
                <p className="text-3xl font-bold text-red-300 mt-2">$2,150</p>
                <p className="text-red-400 text-xs mt-2">1 فاتورة</p>
              </div>
              <AlertCircle className="w-10 h-10 text-red-400/50" />
            </div>
          </div>
        </div>

        {/* Search and Filter */}
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white/40" />
            <input
              type="text"
              placeholder="ابحث عن فاتورة..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-blue-500/50"
            />
          </div>
          <Button variant="outline" className="border-white/20 hover:bg-white/10">
            <Download className="w-4 h-4 mr-2" />
            تصدير
          </Button>
        </div>

        {/* Payments Table */}
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-white/10 border-b border-white/10">
                <tr>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">رقم الفاتورة</th>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">التاريخ</th>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">العميل</th>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">المبلغ</th>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">طريقة الدفع</th>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">الحالة</th>
                  <th className="px-6 py-4 text-right text-sm font-semibold text-blue-200">الإجراءات</th>
                </tr>
              </thead>
              <tbody>
                {payments.map((payment) => (
                  <tr key={payment.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 text-sm font-medium text-white">{payment.id}</td>
                    <td className="px-6 py-4 text-sm text-white/70">{payment.date}</td>
                    <td className="px-6 py-4 text-sm text-white/70">{payment.customer}</td>
                    <td className="px-6 py-4 text-sm font-medium text-white">${payment.amount}</td>
                    <td className="px-6 py-4 text-sm text-white/70">{payment.method}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${payment.statusColor}`}>
                        {payment.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <Button size="sm" variant="outline" className="border-white/20 hover:bg-white/10">
                        <Eye className="w-4 h-4" />
                      </Button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>

        {/* Payment Methods */}
        <div className="mt-8">
          <h3 className="text-2xl font-bold text-white mb-6">طرق الدفع المتاحة</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-blue-600/20 to-blue-700/20 border border-blue-500/30 rounded-2xl p-6">
              <CreditCard className="w-8 h-8 text-blue-400 mb-4" />
              <h4 className="text-lg font-bold text-white mb-2">بطاقة ائتمان</h4>
              <p className="text-blue-200/60 text-sm">دفع آمن عبر بطاقات Visa و Mastercard</p>
            </div>
            <div className="bg-gradient-to-br from-purple-600/20 to-purple-700/20 border border-purple-500/30 rounded-2xl p-6">
              <CreditCard className="w-8 h-8 text-purple-400 mb-4" />
              <h4 className="text-lg font-bold text-white mb-2">تحويل بنكي</h4>
              <p className="text-purple-200/60 text-sm">تحويل مباشر من حسابك البنكي</p>
            </div>
            <div className="bg-gradient-to-br from-green-600/20 to-green-700/20 border border-green-500/30 rounded-2xl p-6">
              <CreditCard className="w-8 h-8 text-green-400 mb-4" />
              <h4 className="text-lg font-bold text-white mb-2">محفظة رقمية</h4>
              <p className="text-green-200/60 text-sm">دفع سريع عبر المحافظ الرقمية</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
