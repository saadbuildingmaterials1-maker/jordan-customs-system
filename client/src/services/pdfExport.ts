...
export async function exportToPDF(data: ExportData) {
  try {
    ...
    // حفظ الملف
    const fileName = `البيان_الجمركي_${data.declaration.declarationNumber}_${Date.now()}.pdf`;
    doc.save(fileName);
...
/**
 * تصدير PDF مع تحميل مباشر (بديل)
 */
export async function exportToPDFWithDownload(data: ExportData) {
  try {
    const doc = new jsPDF({
      orientation: "portrait",
      unit: "mm",
    ...
    // الحصول على البيانات الثنائية للملف
